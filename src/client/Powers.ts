...
class PowerObject extends BaseEntity {
  ...
  constructor(x: number, y: number, type: PowerType, image?: HTMLImageElement) {
    super(x, y, "", 0); // Pass an empty string for color and 0 for speed in BaseEntity
    this.type = type;
    this.image = image;
  }
  //if a image is provided then draw it, else just draw a placeholder
  draw() {
    if (this.image) {
  ...
  //collision checker that checks if a players border are inside the hitbox of the power item
...
// Create new power items and export them
