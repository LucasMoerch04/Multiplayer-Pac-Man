...
import { SpeedObjectCollision, speedObjects, teleportObjectObjectCollision } from "./Powers";

import { buildClientGrid } from "./grid";
import { PacmanAI } from "./pacman_ai";
...
const localPac = new Pacman(70, 70, "red", 6);
...
socket.on("updatePacMan", (pos: { x: number; y: number }) => {
  ...
  if (!frontEndPacMan[0]) {
    frontEndPacMan[0] = new Pacman(pos.x, pos.y, "red", 6);
  } else {
...
// Player list updates
socket.on("updatePlayers", (backendPlayers: Record<string, { x: number; y: number; color: string; speed: number }>) => {
